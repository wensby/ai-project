...
public class SvmTreeNew {
    ...
    public class Flat extends SvmInterface{
        private ArrayList<Svm_model> models = new ArrayList<>();
        private ArrayList<String> model_ft_strings = new ArrayList<>();
        private ArrayList<Integer> svm_weights = new ArrayList<>();
        private int num_svms;
        private String structure_path;

        public Flat(String name) {
            if(name == null || name.equals("")) throw new IllegalArgumentException("No filename provided");
            this.structure_path = "../SvmModels/"+name+"/";
            File path = new File(this.structure_path);
            if (path.exists()){
                load();
            }
        }
        ...
        public void SaveStructure(){
            ...
            saveNumSvms(this.num_svms);
            saveSvms();
            saveWeights();
            saveFeatStrings();
        }

        private void saveNumSvms(int num_svms){

        }

        private void saveSvms(){
        ...
        public void Add(Svm_model model, String feature_string){
            if( !(model == null || feature_string.equals("")) ){
                ...
                this.model_ft_strings.add(feature_string);
                this.svm_weights.add(1);
                this.num_svms ++;
            }
        }
        ...
        public double ClassifySample(Vector<Double> features){
            int sum = 0;
            for(int i = 0; i<this.num_svms; i++){
                sum += svm_weights.get(i)*PredictSingleDataPoint(models.get(i), features);
            }
            double res = 0.0;
            if(sum < 0) res = -1.0;
            if(sum > 0) res = +1.0;
        ...
        public void TrainFromSample(double outcome, Vector<Double> features){
            double res = 0.0;
            for(int i = 0; i<this.num_svms; i++){
                res = PredictSingleDataPoint(models.get(i), features);
